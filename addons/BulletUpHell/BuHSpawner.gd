...
func sanitize_bulletprops(props:PackedDataContainer, id:String, source:Node) -> Dictionary:
	...
	for p:Dictionary in props.get_property_list():
		...
		if P in ["__data__","spec_top_level","spec_ally","mask","r_randomisation_chances",
			...
			"Triggers","Homing","BulletProps.gd","BulletNodeProps.gd","Random","","Color","Trail","Scale"]:
				continue
...
func _plan_spawning(pattern:Pattern, bullets:Array[Dictionary]):
	if pattern.cooldown_next_spawn == 0:
		...
		var to_spawn:Array[Dictionary] = bullets.duplicate()
		for b:Dictionary in to_spawn:
			idx = to_spawn.find(b)
			if pattern.symmetric:
				match pattern.symmetry_type:
...
func plan_spawn(bullets:Array[Dictionary], spawn_delay:float=0):
	#var timestamp:float = getKeyTime(spawn_delay)
...
func plan_shoot(bullets:Array[Dictionary], shoot_delay:float=0):
	#for b in bullets:
		#if not b is RID: bullets.erase(b) # no idea why this exist -> uncomment if issue
	#var timestamp:float = getKeyTime(shoot_delay)
	var insert_index:int = poolTimes.bsearch(time+shoot_delay, false)
	poolTimes.insert(insert_index, time+shoot_delay)
	poolQueue.insert(insert_index, bullets)

#func getKeyTime(delay:float):
	#if loop_length < time+delay: return delay-(loop_length-time)
	#else: return time+delay

func _spawn_and_shoot(to_spawn:Array[Dictionary], to_shoot:Array[Dictionary]):
	_spawn(to_spawn)
	_shoot(to_shoot)
...
func change_animation(b:Dictionary, type:String):
	...
	if not col_id.is_empty() and col_id != b.get("colID", col_id):
		b["colID"] = col_id
		var new_rid:RID = create_shape(b["shared_area"], b["colID"]) # TODO cause shape accumulation ?
		b["RID"] = new_rid
		poolBullets[new_rid] = b
		b["shared_area"].get_meta("Bullets").erase(b["index"])
		b["shared_area"].get_meta("Bullets")[b["shared_area"].get_meta("ShapeCount")-1] = b
		b["index"] = b["shared_area"].get_meta("ShapeCount")-1
		back_to_grave(b["props"]["__ID__"], b)
